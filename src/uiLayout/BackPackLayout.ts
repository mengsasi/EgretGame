...
let UIComponentListTips = 'UIListTips';

class BackPackLayout extends eui.LayoutBase {
	...
	//插入到最高点
	private _insertTop: boolean = false;

	public get insertTop(): boolean {
		return this._insertTop;
	}

	public set insertTop(value: boolean) {
		this._insertTop = value;
	}
	 ...
	 * @private
	 */
	private _horizontalAlign: string = eui.JustifyAlign.JUSTIFY;
	/**
	 ...
	 * @private
	 */
	private _verticalAlign: string = eui.JustifyAlign.JUSTIFY;
	 ...
	 * @private
	 */
	private _columnAlign: string = eui.ColumnAlign.LEFT;
	 ...
	 * @private
	 */
	private _rowAlign: string = eui.RowAlign.TOP;
	/**
	 ...
	 * @private
	 */
	private _orientation: string = eui.TileOrientation.ROWS;
	/**
	 ...
	 * 添加的提示弹框高度
	 */
	private extraElementHeight: number = 0;
	/**
	 * @private
	 * 添加的提示弹框宽度
	 */
	private extraElementWidth: number = 0;
	/**
	 * @private
	 * 添加了Tips
	 */
	private extraExist: boolean = false;
	/**
	 * @private
	 * 当前选中的
	 */
	...
	private calculateExtraElement() {
		let target = this.$target;
		if (!target)
			return;
		this.clearExtraInfo();
		let list = <eui.List>target;
		this.selectedIndex = list.selectedIndex;

		if (this.selectedIndex == -1) {
			return;
		}
		let numChilds = target.numChildren;
		let count = numChilds;
		for (let index = 0; index < count; index++) {
			let child = <eui.UIComponent>(target.getChildAt(index));
			if (child && egret.is(child, UIComponentListTips)) {
				this.extraElementWidth = child.width;
				this.extraElementHeight = child.height;
	...
	private clearExtraInfo() {
		this.extraExist = false;
		this.extraElementWidth = 0;
		this.extraElementHeight = 0;
	}
	...
	public measure(): void {
		...
		this.calculateRowAndColumn(values[eui.sys.UIKeys.explicitWidth], values[eui.sys.UIKeys.explicitHeight]);
	...
	private calculateRowAndColumn(explicitWidth: number, explicitHeight: number): void {
		...
		let orientedByColumns = (this._orientation == eui.TileOrientation.COLUMNS);
	...
	private doUpdateMaxElementSize(startIndex: number, endIndex: number): void {
		...
		if ((startIndex != -1) && (endIndex != -1)) {
			for (let index = startIndex; index <= endIndex; index++) {
				let elt = <eui.UIComponent>target.getVirtualElementAt(index);
	...
	private getIndexInView(): boolean {
		...
		if (this._orientation == eui.TileOrientation.COLUMNS) {
			...
			let maxVisibleX = minVisibleX + values[eui.sys.UIKeys.width];
			...
			let maxVisibleY = minVisibleY + values[eui.sys.UIKeys.height];

			let extra = Math.ceil(this.extraElementHeight / this._rowHeight);
	...
	public updateDisplayList(width: number, height: number): void {
		...
		let selected = this.extraExist;
		let extraX: number;
		let extraY: number;
		let selectedIndex = this.selectedIndex;
		let extraWidth = this.extraElementWidth;
		let extraHeight = this.extraElementHeight;
		let selectedRow = -1;

		let isInsertTop = this._insertTop;
		if (selected) {
			if (isInsertTop) {
				let childCount = target.numChildren;
				for (let i = 0; i < childCount; i++) {
					let tip = <eui.UIComponent>target.getChildAt(i);
					if (tip && egret.is(tip, UIComponentListTips)) {
						switch (this._horizontalAlign) {
							case egret.HorizontalAlign.RIGHT:
								extraX = (width - extraWidth) / 2 - paddingR;
								break;
							case egret.HorizontalAlign.LEFT:
								extraX = (width - extraWidth) / 2 + paddingL;
								break;
							default:
								extraX = (width - extraWidth) / 2 + paddingL;
						}
						...
						this.sizeAndPositionElement(tip, extraX, extraY, extraWidth, extraHeight);
						break;
					}
				}
			}
			else {
				selectedRow = Math.floor(selectedIndex / columnCount);
				let childCount = target.numChildren;
				for (let i = 0; i < childCount; i++) {
					let tip = <eui.UIComponent>target.getChildAt(i);
					if (tip && egret.is(tip, UIComponentListTips)) {
						...
						switch (this._verticalAlign) {
							case egret.VerticalAlign.TOP:
								extraY = (selectedRow + 1) * (rowHeight + verticalGap) + paddingT;
								break;
							case egret.VerticalAlign.BOTTOM:
								extraY = height - (selectedRow + 2) * (rowHeight + verticalGap) + verticalGap - paddingB;
								break;
							default:
								extraY = (selectedRow + 1) * (rowHeight + verticalGap) + paddingT;
						}
		...
		for (let i = this.startIndex; i <= endIndex; i++) {
			if (this.$useVirtualLayout) {
				elt = <eui.UIComponent>(this.target.getVirtualElementAt(i));
			} else {
				elt = <eui.UIComponent>(this.target.getElementAt(i));
			}
		...
		contentHeight += vPadding;
		if (selected) {
			contentHeight += extraHeight;
		}
		target.setContentSize(contentWidth + hPadding, contentHeight);

		if (selected) {
			let list = <eui.List>target;
			if (list['scrollToPosition']) {
				list['scrollToPosition']({ extraY: extraY, extraHeight: extraHeight });
				list['scrollToPosition'] = null;
			}
		}
	}
	...
	getItemHeight() {
		...
		switch (this._verticalAlign) {
			case egret.VerticalAlign.TOP:
				return rowHeight + verticalGap + paddingT;
			case egret.VerticalAlign.BOTTOM:
				return rowHeight + verticalGap + paddingB;
			default:
				return rowHeight + verticalGap + paddingT;
		}
	}
	 ...
	 * 为单个元素布局
	 */
	private sizeAndPositionElement(element: eui.UIComponent, cellX: number, cellY: number,
		...
		else if (!isNaN(values[eui.sys.UIKeys.percentWidth]))
			elementWidth = cellWidth * values[eui.sys.UIKeys.percentWidth] * 0.01;

		if (this._verticalAlign == eui.JustifyAlign.JUSTIFY)
			elementHeight = cellHeight;
		else if (!isNaN(values[eui.sys.UIKeys.percentHeight]))
			elementHeight = cellHeight * values[eui.sys.UIKeys.percentHeight] * 0.01;
